...
C     and
C     Process: t b~ > t b~ [ LOonly = QCD QED ] QCD^2=6 QED^2=0
C     
C     
C     CONSTANTS
C     
...
C     Reshuffle the momenta here
      P_SAVE(:,:) = P(:,:)
      PASS_RESHUFFLE = .TRUE.
      CALL RESHUFFLE_MOMENTA(P,P_RESH,(/1,4/),(/5,-6/),(/6,-5/)
     $ ,PASS_RESHUFFLE)
      P(:,:)=P_RESH(:,:)
...
C     Amplitude(s) for diagram number 1
      CALL FFV1_0(W(1,4),W(1,2),W(1,5),GC_11,AMP2(1))
C     JAMPs contributing to orders QCD=2 QED=0
      JAMP2(1,1) = (5.000000000000000D-01)*AMP2(1)
      JAMP2(2,1) = (-1.666666666666667D-01)*AMP2(1)
